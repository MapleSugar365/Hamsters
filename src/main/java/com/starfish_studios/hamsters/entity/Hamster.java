...
import com.starfish_studios.hamsters.HamstersConfig.BurstStyleEnum;
import com.starfish_studios.hamsters.block.HamsterBowlBlock;
import com.starfish_studios.hamsters.block.HamsterWheelBlock;
import com.starfish_studios.hamsters.entity.util.MMPathNavigatorGround;
import com.starfish_studios.hamsters.entity.util.SleepGoal;
import com.starfish_studios.hamsters.entity.util.SleepingAnimal;
import com.starfish_studios.hamsters.entity.util.SmartBodyHelper;
...
public class Hamster extends TamableAnimal implements GeoEntity, SleepingAnimal {
    ...
    private static final EntityDataAccessor<ItemStack> SEED_IN_MOUTH = SynchedEntityData.defineId(Hamster.class,
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new AvoidEntityGoal<>(this, LivingEntity.class, 6.0F, 1.3D, 1.5D,
                livingEntity -> livingEntity instanceof Player player ? !this.isTame() && !player.isCrouching()
                        : livingEntity.getType().is(HamstersTags.HAMSTER_AVOIDED)));
        ...
        this.goalSelector.addGoal(11, new LookAtPlayerGoal(this, Player.class, 6.0F) {
            @Override
            public void tick() {
                if (Hamster.this.canUseMovementGoals())
                    super.tick();
            }
        });
    ...
    private static final EntityDataAccessor<Integer> SLEEP_COOLDOWN_TICKS = SynchedEntityData.defineId(Hamster.class,
            EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> VARIANT = SynchedEntityData.defineId(Hamster.class,
            EntityDataSerializers.INT),
    ...
    private static final EntityDataAccessor<Boolean> SLEEPING = SynchedEntityData.defineId(Hamster.class,
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(SEED_IN_MOUTH, ItemStack.EMPTY);
    }
    ...
    public void addAdditionalSaveData(@NotNull CompoundTag compoundTag) {
        ...
        if (!seed.isEmpty()) {
            ...
            seed.save(level().registryAccess(), seedTag);
            compoundTag.put("SeedInMouth", seedTag);
        }
    }
    ...
    public void readAdditionalSaveData(@NotNull CompoundTag compoundTag) {
        ...
        if (compoundTag.contains("SeedInMouth")) {
            ...
            HolderLookup.Provider lookupProvider = this.level() != null
                    ? this.level().registryAccess()
                    : RegistryAccess.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY);
            ItemStack stack = ItemStack.parseOptional(lookupProvider, seedTag);
            this.getEntityData().set(SEED_IN_MOUTH, stack);
        }
    }
        ...
        if (this.isSquishing) {
            if (this.squishAnimationTimer > 0) {
                if (this.squishAnimationTimer == 10) {
                    this.playSound(HamstersSoundEvents.HAMSTER_UNSQUISH.get());
                }
        ...
        if (hamstersBurst) {
            if (this.level().isClientSide()) {
                if (this.getCheekLevel() == 2 && this.tickCount % 10 == 0) {
                    this.level().addParticle(ParticleTypes.SPLASH, this.getX(), this.getY(1.2), this.getZ(), 0.0D, 0.2D,
                            0.0D);
                    this.playSound(HamstersSoundEvents.HAMSTER_BEG.get());
                } else if (this.getCheekLevel() == 3 && this.tickCount % 5 == 0) {
                    this.level().addParticle(ParticleTypes.SPLASH, this.getX(), this.getY(1.2), this.getZ(), 0.0D, 0.2D,
                            0.0D);
                }
            } else {
                if (this.isAlive() && this.getCheekLevel() >= 2
                        && this.tickCount % (80 - (this.getCheekLevel() * 10)) == 0)
                    this.playSound(HamstersSoundEvents.HAMSTER_BEG.get());
            }
        }
    }
    ...
    public boolean hurt(@NotNull DamageSource damageSource, float damageAmount) {
        if (this.isTame() && "explosion".equals(damageSource.getMsgId())
                && damageSource.getDirectEntity() instanceof Creeper) {
            return false;
        }
    ...
    public void squishHamster() {
        ...
        if (!this.isSquishing) {
            ...
            this.playSound(HamstersSoundEvents.HAMSTER_SQUISH.get());
        }
    }
                    ...
                    if (this.getAge() < 0) {
                        ...
                        if (this.getCheekLevel() >= 3 && hamstersBurst) {
                            ...
                            this.level().addFreshEntity(
                                    new ExperienceOrb(this.level(), this.getX(), this.getY(), this.getZ(), 3));
                            for (int seedItems = 0; seedItems < 4; seedItems++) {
                                ItemEntity seedsItem = new ItemEntity(this.level(), this.getX(), this.getY(),
                                        this.getZ(), new ItemStack(Items.WHEAT_SEEDS));
    ...
    private void catchHamster(Player player) {
        ItemStack itemStack = new ItemStack(HamstersItems.HAMSTER.get());
        ...
        this.playSound(HamstersSoundEvents.HAMSTER_PICK_UP.get());
    ...
    public AgeableMob getBreedOffspring(@NotNull ServerLevel serverLevel, @NotNull AgeableMob ageableMob) {
        ...
        Hamster hamster = HamstersEntityType.HAMSTER.get().create(serverLevel);
        assert hamster != null;
        if (ageableMob instanceof Hamster hamsterParent) {
            ...
            if (((this.getVariant() == Variant.CHAMPAGNE.getId() && this.getMarking() == Marking.WHITEBELLY.getId()) &&
                    (hamsterParent.getVariant() == Variant.DOVE.getId()
                            && hamsterParent.getMarking() == Marking.SPOTTED.getId()))
                    ||
                    ((this.getVariant() == Variant.DOVE.getId() && this.getMarking() == Marking.SPOTTED.getId()) &&
                            (hamsterParent.getVariant() == Variant.CHAMPAGNE.getId()
                                    && hamsterParent.getMarking() == Marking.WHITEBELLY.getId()))) {
    ...
    public @NotNull SoundEvent getEatingSound(@NotNull ItemStack itemStack) {
        return HamstersSoundEvents.HAMSTER_EAT.get();
    }
    ...
    public class HamsterGoToBlockGoal extends MoveToBlockGoal {
        ...
        public BlockPos getPosWithBlock(BlockPos blockPos, BlockGetter blockGetter) {
            ...
            if (blockGetter.getBlockState(blockPos).is(this.getBlockTag()))
                return blockPos;
            for (BlockPos blocksToCheck : new BlockPos[] { blockPos.below(), blockPos.west(), blockPos.east(),
                    blockPos.north(), blockPos.south(), blockPos.below().below() }) {
    ...
    public class HamsterGoToWheelGoal extends HamsterGoToBlockGoal {
        ...
        public BlockPos getPosWithBlock(BlockPos blockPos, BlockGetter blockGetter) {
            if (blockGetter.getBlockState(blockPos).is(this.getBlockTag())
                    && HamsterWheelBlock.isOccupied((Level) blockGetter, blockPos))
                return null;
            return super.getPosWithBlock(blockPos, blockGetter);
        }
    ...
    public static class HamsterDismountGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            return this.hamster.getVehicle() != null && this.hamster.getDismountingCooldownTicks() <= 0
                    && this.hamster.getCheekLevel() <= 0;
        }
